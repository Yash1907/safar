...
import { createZapplyAdapter } from "./zapply.ts";
import { loadConfig, type RepoSourceConfig, type SafarConfig } from "../config.ts";

/**
 * Builds source adapters from config (§5 M4 — jobright/Simplify/zapply repos are
...
export function buildSources(config: SafarConfig): SourceAdapter[] {
  ...
  return [
    ...
    ...enabled(config.sources.zapply).map((c) =>
      createZapplyAdapter({
        id: c.id,
        displayName: c.displayName,
        owner: c.owner,
